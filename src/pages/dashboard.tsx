...
import { getTestResults } from 'apiSdk/test-results';
import { TestResultInterface } from 'interfaces/test-result';
import { getCiCdIntegrations } from 'apiSdk/ci-cd-integrations';
...
import { ApiListPage as ShowCaseList } from 'pages/apis';

function DashboardPage() {
  const { session } = useSession();

  const testResultStat = useSWR<PaginatedInterface<TestResultInterface>>(
    () => `/test-results`,
    () => getTestResults({ limit: 1, offset: 0 }),
  );

  const ciCdIntegrationStat = useSWR<PaginatedInterface<CiCdIntegrationInterface>>(
    () => `/ci-cd-integrations`,
    () => getCiCdIntegrations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Test Results',
      result: testResultStat?.data?.totalCount,
      isLoading: testResultStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Ci Cd Integrations',
      result: ciCdIntegrationStat?.data?.totalCount,
      isLoading: ciCdIntegrationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Test Runs',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`apis`} passHref legacyBehavior>
                  <Button

...
import { getCiCdIntegrationById } from 'apiSdk/ci-cd-integrations';
import { CiCdIntegrationInterface } from 'interfaces/ci-cd-integration';

function CiCdIntegrationViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Ci Cd Integration Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('ci_cd_integration', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/ci-cd-integrations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Integration Type" text={data?.integration_type} />
                ...
                {hasAccess('api', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Api"
                    text={
                      <Link as={NextLink} href={`/apis/view/${data?.api?.id}`}>
                        {data?.api?.name}
                      </Link>
                    }
                  />
